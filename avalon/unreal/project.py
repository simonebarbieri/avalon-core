...
def create_unreal_project(
    ...
    if os.path.isdir(os.environ.get("AVALON_UNREAL_PLUGIN", "")):
        ...
        if not os.path.isdir(avalon_plugin_path):
            ...
            dir_util.copy_tree(
                os.environ.get("AVALON_UNREAL_PLUGIN"), avalon_plugin_path
            )

            if not os.path.isdir(
                os.path.join(avalon_plugin_path, "Binaries")
    ...
    if os.environ.get("AVALON_UNREAL_INSTALL_UEP"):
        ...
        if not os.path.isdir(
            os.path.join(uep_path, "Binaries")
...
""".format(
    ...
    with open(
        os.path.join(targets_dir, f"{project_name}.Target.cs"), mode="w"
    ) as f:
        f.write(module_target)

    with open(
        os.path.join(targets_dir, f"{project_name}Editor.Target.cs"), mode="w"
    ) as f:
        f.write(editor_module_target)

    with open(
        os.path.join(sources_dir, f"{project_name}.Build.cs"), mode="w"
    ) as f:
        f.write(module_build)

    with open(os.path.join(sources_dir, f"{project_name}.cpp"), mode="w") as f:
        f.write(module_cpp)

    with open(os.path.join(sources_dir, f"{project_name}.h"), mode="w") as f:
        f.write(module_header)

    with open(
        os.path.join(sources_dir, f"{project_name}GameModeBase.cpp"), mode="w"
    ) as f:
        f.write(game_mode_cpp)

    with open(
        os.path.join(sources_dir, f"{project_name}GameModeBase.h"), mode="w"
    ) as f:
        f.write(game_mode_h)

    if platform.system().lower() == "windows":
        u_build_tool = (
            f"{engine_path}/Engine/Binaries/DotNET/" "UnrealBuildTool.exe"
        )
        u_header_tool = (
            f"{engine_path}/Engine/Binaries/Win64/" f"UnrealHeaderTool.exe"
        )
    elif platform.system().lower() == "linux":
    ...
    command2 = [
        u_build_tool,
        f"-ModuleWithSuffix={project_name},3555" "Win64",
        "Development",
        "-TargetType=Editor" f'-Project="{project_file}"',
        f'"{project_file}"' "-IgnoreJunk",
    ]
